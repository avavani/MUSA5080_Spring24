...
data_split<- zillow%>%dplyr::select(USER_ID)
...
data_joined <- data_frame(USER_ID = word_rownames) %>%
  left_join(zillow %>%
              dplyr::select(USER_ID, cluster1, cluster2, cluster3, cluster4b))
...
comment_classes <- classifications %>%
  left_join(zillow %>%
              dplyr::select(cluster1, USER_ID), by = "USER_ID") %>% #change here to clusterX 
