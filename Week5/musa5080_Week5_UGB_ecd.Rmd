...
tracts17 <- tracts17 %>%
  mutate(sq_miles = area_sqFt/27878400)%>%
  mutate(popdens = TotalPop/sq_miles)

ggplot()+
  geom_sf(data = tracts17, aes(fill=popdens), color = "transparent")+
  scale_fill_distiller(palette = "YlOr", direction = 1, breaks = "jenks")+
  geom_sf(data = uga, fill = NA, color = "maroon", lwd = 0.25, linetype = 1)+
  labs(fill = "Population per Sq Miles",
       title = "Population Density in Lancaster County")+
  theme_void()
```
...
map_dens <- tm_shape(tracts17) +
  tm_polygons(col = "popdens", style = "quantile", border.col = NA, n=6,
              title = "Population Density", palette = "YlGnBu")+
  tm_layout(frame = FALSE,legend.frame = FALSE,
            title = "Population Density of Census Tracts in Lancaster County",
            title.size = 0.5)+
  tm_shape(uga)+
...
```{r buffer_plot_tmap}
tm_shape(bothBuffers)+
  tm_polygons(col = "Legend", palette = c("#F8766D", "#00BFC4"))+
  tm_layout(legend.show = TRUE, title = "1/8mi Buffer Inside & Outside UGA",
            title.size = 0.7)
```
...
```{r}
tracts_centroid <- st_centroid(tracts17)
outside_polygons <- st_union(tracts17)%>%
  st_as_sf()

outside_polygons <- outside_polygons %>%
  st_difference(uga_union)%>%
  st_as_sf()%>%
  mutate(Legend = "Outside")
...
two_polygons <- rbind(outside_polygons, uga_union)

joined_tracts <- tracts_centroid %>%
  st_join(two_polygons)

buffer_tracts <- joined_tracts %>%
  group_by(Legend)%>%
  summarize(People_per_sq_mile = mean(popdens))%>%
  st_drop_geometry()
...
```{r}
ggplot()+
  geom_sf(data = joined_tracts, aes(fill=Legend), colour = "maroon")+
  geom_sf(data = bu)
